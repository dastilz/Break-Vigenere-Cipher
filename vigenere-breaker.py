letter_frequency = {
    'e': 11.1607,
    'a': 8.4966,
    'r': 7.5809,
    'i': 7.5448,
    'o': 7.1635,
    't': 6.9509,
    'n': 6.6544,
    's': 5.7351,
    'l': 5.4893,
    'c': 4.5388,
    'u': 3.6308,
    'd': 3.3844,
    'p': 3.1671,
    'm': 3.0129,
    'h': 3.0034,
    'g': 2.4705,
    'b': 2.0720,
    'f': 1.8121,
    'y': 1.7779,
    'w': 1.2899,
    'k': 1.1016,
    'v': 1.0074,
    'x': 0.2902,
    'z': 0.2722,
    'j': 0.1965,
    'q': 0.1962
}

raw_text = ''
ALPHABET_LETTERS = 26

def generateSubStrings(text):
    arr = {}

    for i in range(0, len(raw_text)-3):
        arr[i] = text[i:i+4]
    
    return arr

def findMatches(map):
    matches = []

    for key1, mapped1 in map.items():
        for key2, mapped2 in map.items():
            if key1 != key2 and mapped1 == mapped2:
                matches.append([key1, key2])

    return matches

def guessKey(matches):
    matchDifferences = []
    for i, j in matches:
        matchDifferences.append(abs(i-j))
    
    print(matchDifferences)

    # Assume the key is either 2, 3, 4, 5, or 6 (1 would just be a Caesar cipher)

    

substrings = generateSubStrings(raw_text)
matches = findMatches(substrings)
guessKey(matches)